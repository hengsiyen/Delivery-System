      ...
      <div class="card">
        ...
        <div class="card-body">
          ...
          <div class="row">
            <div class="col-lg-12">
              <div class="form-group row">
                <label class="required col-lg-3 col-form-label text-lg-right">{{ $t('label.code') }}</label>
                <div class="col-lg-9">
                  <input
                    v-model="code"
                    type="text"
                    class="form-control"
                    :class="{'is-invalid': checkValidate('code')}"
                    :placeholder="$t('pla.code')"
                  >
                  <small v-if="checkValidate('code')" class="invalid-feedback">
                    {{ validate.code[0] }}
                  </small>
                </div>
              </div>
            ...
            <div class="col-lg-12">
              <div class="form-group row">
                <label class="required col-lg-3 col-form-label text-lg-right">{{ $t('label.symbol') }}</label>
                <div class="col-lg-9">
                  <input
                    v-model="symbol"
                    type="text"
                    class="form-control"
                    :class="{'is-invalid': checkValidate('symbol')}"
                    :placeholder="$t('pla.symbol')"
                  >
                  <small v-if="checkValidate('symbol')" class="invalid-feedback">
                    {{ validate.symbol[0] }}
                  </small>
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    storeOrEdit () {
      this.$isLoading(true)
      this.validate = null
      this.$axios.post(this.$base_api + '/api/backend/currency/store-or-edit', {
        id: this.id,
    ...
    getCurrency () {
      this.$axios
        ...
        .then((res) => {
          ...
          this.code = this.oldData.code
          this.symbol = this.oldData.symbol
