  ...
  <div class="card">
    <div class="card-header with-border">
      ...
      <div class="card-tools">
        <ButtonAddNew link-to="create-currency" />
      </div>
    </div>
    <div class="card-body">
      <Datatable
        ...
        url="api/backend/currency/datatable"
      />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    toggleRecode (id) {
      this.$isLoading(true)
      this.$axios.post(this.$base_api + '/api/backend/currency/toggle', { id })
        .then((res) => {
          if (this.$refs.oTable) {
    ...
    deleteRecord (id) {
      this.$swal({
        html: `<label class="mb-3 font-s-20">${this.$t('swal.delete_currency')}</label>`,
        icon: 'warning',
